...
class JutsuClassifier():
    def __init__(self,
        ...
        if not huggingface_hub.repo_exists(self.model_path):
            ...
            if data_path is None:
                raise ValueError("Data path must be provided if the model, since the model path does not exist.")
    ...
    def load_data(self, data_path):
        ...
        tokenize_train = train_dataset.map(lambda examples: self.preprocess_function(self.tokenizer, examples),batched=True)
        tokenize_test = test_dataset.map(lambda examples: self.preprocess_function(self.tokenizer, examples),batched=True)

        return tokenize_train, tokenize_test 
